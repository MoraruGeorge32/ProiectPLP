...
Notation "A 'sau' B" := (bor A B)(at level 53, right associativity).
...
Notation "A ;; B" := (sequence A B)(at level 60, right associativity).
Notation "'while''(' A ')''(' B ')'" := (while A B)(at level 90).
Notation " 'iff' A 'then' B 'else' C " := (ifthenelse A B C)(at level 90).
Notation " 'iff' A  'then  B " := (ifthen A B)(at level 90).

Check btrue.
Check "camasa" <n> 3.
Check while ( 3 <' 5 )( "camasa" <n> 3 ;; "nasture" <s> "Copilot" ).
Check iff (3 <'5) then "camasa" <-n 3  else "nasture" <s> "copilot".
